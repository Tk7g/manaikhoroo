	...
	<div class="box span11">
		<div class="box-header" data-original-title>
			...
			<?php if($_SESSION['login'] == 2) { 
				...
				<i class="halflings-icon user"></i><span class="break"></span><?php echo $results['year'].' оны '.$results['data']['district_title'].' дүүргийн өрхийн тоо'; ?>
			</h2>
			<div class="box-icon">
...
<script type="text/javascript">
	...
	$(function() {
		...
		<?php foreach($results['regions'] as $region) : ?>
			["<?php echo $region['title']; ?>", <?php if($results['region_data'][$region['id']] != NULL) { echo $results['region_data'][$region['id']]['household']; } else { echo 0; } ?>],
		<?php endforeach; ?>
		];
    	...
    	$("#chartHousehold").on("plothover", function (event, pos, item) {
        	if (item) {
            	if ((previous_point != item.dataIndex) || (previous_label != item.series.label)) {
                	...
                	show_tooltip(item.pageX, item.pageY, "<div style='text-align: center;'><?php echo $district_title.' дүүрэг'; ?><br/>" + (x+1) + "-р хорооны<br/>Нийт өрхийн тоо<br /><b><div style='font-size: 18px; color: #000;'>" + y + "</div></b></div>");
            	}
        	} else {
				$("#bar_tooltip").remove();
            	previous_point = null;
