...
class User {
	...
	public function registerUser($user) {
			...
			if ($user_data == NULL) {
				...
				$primary_id = array('password' => $pass, 'username' => $data['username'], 'email' => $data['email']);
			} else {
				$primary_id = 0;
			}
			$db->close();
	...
	public function login($user) {
		...
		if ($login_data != NULL) {
			session_regenerate_id(true);
			$_SESSION['login'] = array('username' => $data['username'], 'group_id' => $login_data['group_id'], 'district_id' => $login_data['district_id'], 'region_id' => $login_data['region_id'], 'email' => $login_data['email'], 'identity' => $login_data['identity'], 'firstname' => $login_data['firstname'], 'lastname' => $login_data['lastname']);
			$_SESSION['start'] = time();
