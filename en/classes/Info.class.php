...
class Info {
	...
	public function sectionInfoAdd($district, $region, $info) {
		$db = new Database( DB_SERVER, DB_USER, DB_PASS, DB_DATABASE );
		$db->connect();
		...
		$sections = $this->getSections($db, $region);
		$regionData = array('population' => 0, 'household' => 0, 'household_average' => 0, 'area_length' => 0, 'population_density' => 0, 'kin_num' => 0, 'tot_kinnum' => 0, 'kin_ratio' => 0, 'school_num' => 0, 'tot_schoolnum' => 0, 'school_ratio' => 0, 'year' => $info['year'], 'region_id' => $region, 'district_id' => $district, 'pale_ground' => 0, 'trash_collect' => 0);
		$sec_count = 0;
		foreach($sections as $sec) {
			$section_data = $this->getSectionData($db, $region, $district, $sec['title'], $info['year']);
			if($section_data != NULL) {
				$regionData['population'] = $regionData['population'] + $section_data['population'];
				$regionData['household'] = $regionData['household'] + $section_data['household'];
				$regionData['household_average'] = $regionData['household_average'] + $section_data['household_average'];
				$regionData['area_length'] = $regionData['area_length'] + $section_data['area_length'];
				$regionData['population_density'] = $regionData['population_density'] + $section_data['population_density'];
				$regionData['bus_density'] = $regionData['bus_density'] + $section_data['bus_density'];
				$regionData['well_density'] = $regionData['well_density'] + $section_data['well_density'];
				$regionData['risk_ratio'] = $regionData['risk_ratio'] + $section_data['risk_ratio'];
				$regionData['kin_num'] = $regionData['kin_num'] + $section_data['kin_num'];
				$regionData['tot_kinnum'] = $regionData['tot_kinnum'] + $section_data['tot_kinnum'];
				$regionData['school_num'] = $regionData['school_num'] + $section_data['school_num'];
				$regionData['tot_schoolnum'] = $regionData['tot_schoolnum'] + $section_data['tot_schoolnum'];
				$regionData['pale_ground'] = $regionData['pale_ground'] + $section_data['pale_ground'];
				$regionData['trash_collect'] = $regionData['trash_collect'] + $section_data['trash_collect'];
				$regionData['toron_garts'] = $regionData['toron_garts'] + $section_data['toron_garts'];
				$sec_count = $sec_count + 1;	
			}
		}
		$regionData['household_average'] = $regionData['population']/$regionData['household'];
		$regionData['population_density'] = $regionData['population']/$regionData['area_length'];
		$regionData['bus_density'] = $regionData['bus_density']/$sec_count;
		$regionData['well_density'] = $regionData['well_density']/$sec_count;
		$regionData['risk_ratio'] = $regionData['risk_ratio']/$sec_count;
		$regionData['trash_collect'] = $regionData['trash_collect']/$sec_count;
		$regionData['kin_ratio'] = $regionData['kin_num']*100/$regionData['tot_kinnum'];
		$regionData['school_ratio'] = $regionData['school_num']*100/$regionData['tot_schoolnum'];
		/* Усны худгийн тоо */
		$regionData['well_num'] = $this->markerCount($db, 2, $info['year'], $district, $region);
		/* 1000 хүнд ноогдох усны худгийн харьцаа */
		$regionData['well_ratio'] = ($regionData['well_num']/$regionData['population'])*1000;
		/* Албан бус хогийн цэгийн тоо */
		$regionData['trash_num'] = $this->markerCount($db, 10, $info['year'], $district, $region);
		/* Өрхийн болон бусад эмнэлэгийн тоо */
		$regionData['hospital_num'] = $this->markerCount($db, 9, $info['year'], $district, $region);
		/* 1000 хүнд ноогдох өрхийн болон бусад эмнэлэгийн харьцаа */
		$regionData['hospital_ratio'] = ($regionData['hospital_num']/$regionData['population'])*1000;
		$regionDataExist = $this->getRegionData($db, $district, $region, $info['year']);
		if($regionDataExist != NULL) {
			$regionData['modified'] = date('Y-m-d H:i:s');
			$regionId = $db->query_update(TABLE_INFOS, $regionData, "id='".$regionDataExist['id']."'");
		} else {
			$regionData['district_id'] = $district;
			$regionData['region_id'] = $region;
			$regionData['year'] = $info['year'];
			$regionData['created'] = date('Y-m-d H:i:s');
			$regionData['modified'] = date('Y-m-d H:i:s');
			$regionId = $db->query_insert(TABLE_INFOS, $regionData);
		}
	...
	public static function sectionInfoEdit($district, $region, $info) {
		$db = new Database( DB_SERVER, DB_USER, DB_PASS, DB_DATABASE );
		$db->connect();
		$data['region_id'] = $region;
		$data['district_id'] = $district;
