	  ...
	  * 22.06.2010
	  */
	 function PolygonCreator(map, RoadWidth){
	 	 //create pen to draw on map
		 this.map = map;
		 this.pen = new Pen(this.map, RoadWidth);
		 var thisOjb=this;
	 ...
	 function Pen(map, RoadWidth){
			...
			if (null != this.polygon) {
				...
				if (this.currentDot != null && this.listOfDots.length > 1 && this.currentDot == this.listOfDots[0]) {
					...
					if(this.listOfDots.length > 1){
						this.polyline = new Line(this.listOfDots, this.map, RoadWidth);
					}
				}
			}
		}
		...
		this.drawPloygon = function (listOfDots,color,des,id){
			this.polygon = new Polygon(listOfDots,RoadWidth,this.map,this,color,des,id);
			this.deleteMis();
		}
	  ...
	  * Line class
	  */
	 function Line(listOfDots,map,RoadWidth){
	  ...
	  * polygon class
	  */
	 function Polygon(listOfDots,RoadWidth,map,pen,color){
		this.listOfDots = listOfDots;
		this.map = map;
		this.coords = new Array();
		this.parent = pen;
