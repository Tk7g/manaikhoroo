...
<div class="row-fluid sortable">
	<div class="box span11">
		<div class="box-header" data-original-title>
			<h2>
				<i class="halflings-icon user"></i><span class="break"></span><?php echo $results['data']['district_title'].' дүүргийн '.$results['data']['region_title'].'-р хорооны өрхийн тоо'; ?>
			</h2>
			<div class="box-icon">
		...
		<div class="box-content">
			<div id="chartHousehold" style="width: 100%; height: 200px;"></div>
		</div>
	</div><!--/span-->
</div>
...
<script type="text/javascript">
	...
	$(function() {
		...
		<?php foreach($sections as $sec) : ?>
			["<?php echo $sec['title']; ?>", <?php if($sectionInfos[$sec['id']] != NULL) { echo $sectionInfos[$sec['id']]['household']; } else { echo 0; } ?>],
		<?php endforeach; ?>
		];
		$.plot("#chartHousehold", [ data ], {
			series: {
				bars: {
					show: true,
					barWidth: 0.8,
    	...
    	$("#chartHousehold").on("plothover", function (event, pos, item) {
        	if (item) {
            	if ((previous_point != item.dataIndex) || (previous_label != item.series.label)) {
                	...
                	show_tooltip(item.pageX, item.pageY, "<div style='text-align: center;'><?php echo $results['data']['district_title'].' дүүрэг '.$results['data']['region_title'].'-р хороо'; ?><br/>" + (x+1) + "-р хэсгийн<br/>Өрхийн тоо<br /><b><div style='font-size: 18px; color: #000;'>" + y + "</div></b></div>");
            	}
        	} else {
				$("#bar_tooltip").remove();
            	previous_point = null;
