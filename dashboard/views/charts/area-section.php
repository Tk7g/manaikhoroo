...
<script type="text/javascript">
	...
	$(function() {
		...
		<?php foreach($sections as $sec) : ?>
			["<?php echo $sec['title']; ?>", <?php if($sectionInfos[$sec['id']] != NULL) { echo $sectionInfos[$sec['id']]['area_length']; } else { echo 0; } ?>],
		<?php endforeach; ?>
		];
		$.plot("#chartArea", [ data ], {
			series: {
    	...
    	$("#chartArea").on("plothover", function (event, pos, item) {
        	if (item) {
            	if ((previous_point != item.dataIndex) || (previous_label != item.series.label)) {
                	...
                	show_tooltip(item.pageX, item.pageY, "<div style='text-align: center;'><?php echo $results['data']['district_title'].' дүүрэг '.$results['data']['region_title'].'-р хороо'; ?><br/>" + (x+1) + "-р хэсгийн<br/>газар нутгийн хэмжээ<br /><b><div style='font-size: 18px; color: #000;'>" + y + "</div></b></div>");
            	}
        	} else {
				$("#bar_tooltip").remove();
            	previous_point = null;
