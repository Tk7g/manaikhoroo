...
<div class="row-fluid sortable">
	<div class="box span11">
		<div class="box-header" data-original-title>
			<h2>
			<i class="halflings-icon user"></i><span class="break"></span><?php echo $results['data']['district_title'].' дүүргийн '.$results['data']['region_title'].'-р хорооны хүн амын тоо'; ?>
			</h2>
			<div class="box-icon">
...
<script type="text/javascript">
	...
	$(function() {
		...
		<?php foreach($sections as $sec) : ?>
			["<?php echo $sec['title']; ?>", <?php if($sectionInfos[$sec['id']] != NULL) { echo $sectionInfos[$sec['id']]['population']; } else { echo 0; } ?>],
		<?php endforeach; ?>
		];
    	...
    	$("#chartPopulation").on("plothover", function (event, pos, item) {
        	if (item) {
            	if ((previous_point != item.dataIndex) || (previous_label != item.series.label)) {
                	...
                	show_tooltip(item.pageX, item.pageY, "<div style='text-align: center;'><?php echo $results['data']['district_title'].' дүүрэг '.$results['data']['region_title'].'-р хороо'; ?><br/>" + (x+1) + "-р хэсгийн<br/>Хүн амын тоо<br /><b><div style='font-size: 18px; color: #000;'>" + y + "</div></b></div>");
            	}
        	} else {
				$("#bar_tooltip").remove();
            	previous_point = null;
