...
<?php 
	foreach($results['regions'] as $region) : 
		...
		foreach($regionLines as $regionLine) {
			$data['coordinate'][$k] = explode(',', $regionLine['coordinate'], 2);
			$k = $k + 1;
		}
		header('Content-type: application/json');
		$encoded_line = json_encode($data);
?>
	addRegion(<?php echo $region['id']; ?>, '<?php echo MAIN_FOLDER.''.$region['image']; ?>', <?php echo $encoded_line; ?>);
<?php endforeach; ?>
...
function addRegion(region, image, point) {
