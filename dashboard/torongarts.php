...
function torongartsList() {
	...
	$results = array();
	$data = Torongarts::getRegionList($_SESSION['login']['region_id']);
...
function torongartsAreaCalculate() {
	User::allowExecute(3);
	$districts = District::getDistrictList();
	$region = Region::getRegion($_SESSION['login']['region_id']);
	$district = District::getDistrict($_SESSION['login']['district_id']);
	$sections = Torongarts::getRegionTorongarts($_SESSION['login']['region_id']);
	require( ADMIN_TEMPLATE . "torongarts/torongartsAreaCalculate.php" );
}
