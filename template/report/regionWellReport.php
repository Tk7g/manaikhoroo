...
<div id="reportPage">
	<div class="container">
		<div class="row">
			<div class="col-md-12">
			<img src="<?php echo MAIN_FOLDER.'/images/reportProfile/bg-blue2.png'; ?>" class="bg-page-image"/>
			<div class="page-report">
				...
				<a class="reportBtn" href="http://FreeHTMLtoPDF.com/?convert=http%3A%2F%2Fmanaikhoroo.ub.gov.mn%2Freport.php%3Faction%3DregionWellReportPdf%26district=<?php echo $_GET['district']; ?>%26region=<?php echo $_GET['region']; ?>%26year=<?php echo $_GET['year']; ?>&size=A4&orientation=portrait&enablejs=1">
				...
				<div class="reportMapLarge">
					...
					<div class="reportMapTitle">
						...
						<?php echo $district['title'].' дүүргийн '.$region['title'].'-р хорооны усны хүртээмж'; ?>
						</div>
					</div>
						...
						<div class="reportDetailInfo">
							...
							<div class="reportDetailRow">
								<div class="reportDetailImage">
									<img src="<?php echo MAIN_FOLDER."/images/reportProfile/well.png"; ?>" />
								</div>
				...
				<div class="reportBottom">
					<img src="<?php echo MAIN_FOLDER.'/images/reportProfile/well-bottom.jpg'; ?>" class="reportBottomImg" />
					<div class="reportGraph">
						<div class="reportGraphTitle"><?php echo $district['title'].' дүүргийн хороодуудын '.$_GET['year'].' оны усны худгийн харьцуулалт'; ?></div>
						<div>
					...
					<div class="reportBottomInfo">
						<div class="reportBottomCount">
							<?php echo number_format($info['well_density']); ?>
						</div>
						<div class="reportBottomTitle">
							Хүн амын <?php echo number_format($info['well_density']); ?>% нь усны худагтай ойр 5 минут алхах зайнд амьдардаг.
						</div>
						<div class="reportBottomCount">
							<?php echo number_format($count_marker['COUNT(*)']/$info['population']*1000, 1); ?>
						</div>
						<div class="reportBottomTitle">
							1000 хүнд <?php echo number_format($count_marker['COUNT(*)']/$info['population']*1000, 1); ?> усны худаг ноогддог.
						</div>
					</div>
				</div>
			</div>
...
<?php }} ?>
	...
	addMarker(<?php echo $region['id']; ?>, 2, '<?php echo MAIN_FOLDER."/images/reportProfile/well.png"; ?>', mapLarge, iconBus, <?php echo $_GET['year']; ?>);
	...
	setTimeout(function () {
    	addWalk(<?php echo $region['id']; ?>, "#f0e5d8", 400, 0.15, 2, mapLarge);   
    }, 1000);
	setTimeout(function () {
    	addWalk(<?php echo $region['id']; ?>, "#cdd3c0", 300, 0.2, 2, mapLarge);    
    }, 1500);
	setTimeout(function () {
    	addWalk(<?php echo $region['id']; ?>, "#8ba686", 200, 0.3, 2, mapLarge);    
    }, 2500);
    setTimeout(function () {
    	addWalk(<?php echo $region['id']; ?>, "#195734", 100, 0.4, 2, mapLarge);    
    }, 3400);
    
	google.maps.event.addListener(mapLarge, 'zoom_changed', function() {
		
    	removeMarker(<?php echo $region['id']; ?>, 2, iconBus);
    	addMarker(<?php echo $region['id']; ?>, 2, '<?php echo MAIN_FOLDER."/images/reportProfile/well.png"; ?>', mapLarge, iconBus, <?php echo $_GET['year']; ?>);
...
function addMarker(region, type, image, mapVar, iconVar, year) {
	$.getJSON('<?php echo SITE_URL; ?>report.php?action=getRegionMarkers&type='+type+'&region='+region+'&year='+year, function(point){
		if(point.coordinate.length == 0) {
			return;
		} else {
			if(type != 7) {
