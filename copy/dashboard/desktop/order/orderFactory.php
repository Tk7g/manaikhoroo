...
<div id="mainComponent">
	...
	<div class="mainWrapper">
		...
		<table width="100%" class="orderTable">
			...
			<tr>
				<td>
					<a href="order.php?action=orderFactoryInfo&id=<?php echo $order['id']; ?>">
						<div class="orderStatus" style="background: <?php echo getStatusColor($order['status']); ?>"></div>
					</a>
				</td>
				<td>
					<a href="order.php?action=orderFactoryInfo&id=<?php echo $order['id']; ?>"><?php echo $order['company_name']; ?></a>
		...
		<div id="paginationBlock">
			<?php for($i=1; $i<=$total_pagination; $i++) { ?>
				<?php if(isset($_GET['company'])) { ?>
					<a class="paginationLink" href="order.php?action=orderFactory&page=<?php echo $i; ?>&company=<?php echo $_GET['company']; ?>&product=<?php echo $_GET['product']; ?>"><?php echo $i; ?></a>
				<?php } else { ?>
					<a class="paginationLink" href="order.php?action=orderFactory&page=<?php echo $i; ?>"><?php echo $i; ?></a>
				<?php } ?>
			<?php } ?>
		</div>
	</div>
