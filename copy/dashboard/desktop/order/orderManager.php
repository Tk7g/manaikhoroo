...
<div id="mainComponent">
	...
	<div class="mainWrapper">
		...
		<div class="searchBlock">
				<form action="order.php?action=searchOrderManager" method="post">
			<div class="row">
		...
		<table width="100%" class="orderTable">
			...
			<tr>
				<td>
					<a href="order.php?action=orderInfoManager&id=<?php echo $order['id']; ?>">
						<div class="orderStatus" style="background: <?php echo getStatusColor($order['status']); ?>"></div>
					</a>
				</td>
				<td>
					<a href="order.php?action=orderInfoManager&id=<?php echo $order['id']; ?>"><?php echo $order['company_name']; ?></a>
		...
		<div id="paginationBlock">
			<?php for($i=1; $i<=$total_pagination; $i++) { ?>
				<?php if(isset($_GET['status'])) { ?>
					<a class="paginationLink" href="order.php?action=orderManager&page=<?php echo $i; ?>&status=<?php echo $_GET['status']; ?>&company=<?php echo $_GET['company']; ?>&product=<?php echo $_GET['product']; ?>"><?php echo $i; ?></a>
				<?php } else { ?>
				<a class="paginationLink" href="order.php?action=orderManager&page=<?php echo $i; ?>"><?php echo $i; ?></a>
				<?php } ?>
			<?php } ?>
		</div>
	</div>
