...
class Section {
	...
	public static function calculateArea($section) {
		...
		$db->query_update("kh_sections", $data, "id='".$section['id']."'");
		$db->close();
	}
	
	public static function getSectionArea($section, $region, $district) {
		$db = new Database( DB_SERVER, DB_USER, DB_PASS, DB_DATABASE );
		$db->connect();
		$sql = "SELECT area FROM kh_sections WHERE title = $section AND district_id = $district AND region_id = $region";
		$rows = $db->query_first($sql);
		$db->close();
		return $rows;
	}
	
	public static function getSec($id) {
		$db = new Database( DB_SERVER, DB_USER, DB_PASS, DB_DATABASE );
		$db->connect();
	...
	public static function getRegionPublishedDistinctSections($region) {
		$db = new Database( DB_SERVER, DB_USER, DB_PASS, DB_DATABASE );
		$db->connect();
		$sql = "SELECT DISTINCT(title) FROM kh_sections
				WHERE region_id LIKE ".$region." AND published = 1 ORDER BY title ASC";
		$rows = $db->fetch_all_array($sql);
		$db->close();
		return $rows;
	}
	
	public static function getRegionPublishedSections($region) {
		$db = new Database( DB_SERVER, DB_USER, DB_PASS, DB_DATABASE );
		$db->connect();
		$sql = "SELECT * FROM kh_sections
				WHERE region_id LIKE ".$region." AND published = 1 ORDER BY title ASC";
	...
	public static function confirmSection($sectionId) {
		$db = new Database( DB_SERVER, DB_USER, DB_PASS, DB_DATABASE );
		$db->connect();
		$data['published'] = 1; 
		$data['modified'] = date('Y-m-d H:i:s');
		$db->query_update("kh_sections", $data, "id='".$sectionId."'");
		$db->close();
	}
	
	public static function cancelSection($sectionId) {
		$db = new Database( DB_SERVER, DB_USER, DB_PASS, DB_DATABASE );
		$db->connect();
		$data['published'] = 0; 
		$data['modified'] = date('Y-m-d H:i:s');
	...
	public static function getSectionList($region) {
		$db = new Database( DB_SERVER, DB_USER, DB_PASS, DB_DATABASE );
		$db->connect();
		$sql = "SELECT kh_sections.*, kh_districts.title as districtTitle, kh_districts.id as districtId, kh_regions.title as regionTitle, kh_regions.id as regionId 
				FROM kh_sections
				...
				ORDER BY kh_sections.title ASC";
		$results = $db->fetch_all_array($sql);
		$db->close();
		return $results;
	}
